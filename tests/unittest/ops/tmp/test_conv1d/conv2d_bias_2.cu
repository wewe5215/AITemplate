...
using f86bc4369b4a2fbd5eb4f25b95769e39db094b3d8 = cutlass::conv::device::ImplicitGemmConvolution<cutlass_tensorop_bf16_s16816fprop_optimized_bf16_64x64_32x10_nhwc_single_group_align8_base>;
...
void conv2d_bias_2 (
  ...
  *out_ch = CO;
  ...
  if (NI == 4 && HI == 28 && WI == 1 && CI == 80) {
    
    using ElementComputeEpilogue = typename f86bc4369b4a2fbd5eb4f25b95769e39db094b3d8::ElementCompute;
    //  TODO: cast to right dtype
    typename f86bc4369b4a2fbd5eb4f25b95769e39db094b3d8::Arguments arguments{
        problem_size,                                                                 // ConvProblemSize const & problem_size
        {static_cast<cutlass::bfloat16_t*>(in_ptr), layout_A},                                  // TensorRefA const & ref_A
        {static_cast<cutlass::bfloat16_t*>(weight_ptr), layout_B},                              // TensorRefA const & ref_B

        {static_cast<cutlass::bfloat16_t*>(bias_ptr), cutlass::layout::TensorNHWC::Stride(0)},  // TensorRefC const & ref_C

        {static_cast<cutlass::bfloat16_t*>(out_ptr), layout_C},                                 // TensorRefC const & ref_D

        {ElementComputeEpilogue(1), ElementComputeEpilogue(1)},                       // typename EpilogueOutputOp::Params const & output_op

    };
    f86bc4369b4a2fbd5eb4f25b95769e39db094b3d8 conv_op;
