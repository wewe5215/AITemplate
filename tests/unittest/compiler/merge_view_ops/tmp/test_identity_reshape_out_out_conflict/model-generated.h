...
class Model : public ModelBase<Model> {
  ...
  public:
    ...
    auto* blob_ptr = static_cast<uint8_t*>(blob_.get());
     ...
     params_[1].shape_ptrs = {ParamDim(2, 2, &x0_dim_0), ParamDim(8, 8, &x0_dim_2)};
     params_[2].shape_ptrs = {ParamDim(2, 2, &y1_dim_0), ParamDim(8, 8, &y1_dim_1)};
...
if (params_[2].ptr == nullptr) {
  ...
  DEVICE_CHECK(DeviceToDeviceCopy(params_[2].ptr, y0, 1*2*8 * 4, stream));
    }
    ...
    void ProfileImpl(StreamType stream, size_t iters, const std::string& filename) {
      ...
      {
        std::cout << "Profiling: " << "reduce_sum_0" << " (" << iters << " iterations)" << std::endl;
