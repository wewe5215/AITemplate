...
__global__ void reduce_sum_0_kernel(
...
void reduce_sum_0_launcher(
    ...
    reduce_sum_0_kernel<ElemOutputType, ElemInputType, ElementCompute, VecType>
...
void reduce_sum_0(
  ...
  if (shape[rank - 1] % 4 == 0) {
    reduce_sum_0_launcher<float, float, float, uint4>
      (dst_ptr, src_ptr, reduction_axis, shape, rank, stream);
    return;
  }
  if (shape[rank - 1] % 2 == 0) {
    reduce_sum_0_launcher<float, float, float, uint2>
      (dst_ptr, src_ptr, reduction_axis, shape, rank, stream);
    return;
  }
  if (shape[rank - 1] % 1 == 0) {
    reduce_sum_0_launcher<float, float, float, unsigned>
      (dst_ptr, src_ptr, reduction_axis, shape, rank, stream);
    return;
  }
  throw std::runtime_error(
    "Unsupported workload for this reduce_sum_0 specialization."
  );
}