...
void concatenate_7(
  ...
  for (int64_t i = 0; i < rank; i++) {
    ...
    for (int64_t j = 1; j < num_real_inputs; j++) {
      if (real_input_shapes[j][i] != dim) {
        throw std::runtime_error(
          "invalid input shape, func_name: concatenate_7, dim: " +
  ...
  if (!output) {
    ...
    const TensorAccessor *input_accessors[5] = {

      &input_accessor0, &input_accessor1, &input_accessor2, &input_accessor3, &input_accessor4
  ...
  invoke_concatenate_fast<half, half, 5, 2, transform_type>(
      real_input_shapes,
      inputs,
      input_accessors,
      local_output_shape,
