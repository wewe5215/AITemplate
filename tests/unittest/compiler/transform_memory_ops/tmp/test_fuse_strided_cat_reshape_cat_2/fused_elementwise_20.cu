...
const int N_ELEMENTS_PER_THREAD = sizeof(uint) / sizeof(half);
const int N_ELEMENTS_PER_READ = sizeof(uint) / sizeof(half);
const int N_OPS_PER_THREAD = sizeof(uint) / sizeof(half2);
...
__global__ void
fused_elementwise_20(uint* output0, const uint* input0,const uint* input1, int64_t batch_0,   int64_t n_elements) {
  ...
  uint *input_tmp0 = const_cast<uint*>(input0);
  constexpr int vec_size0 =  sizeof(uint) / sizeof(uint);
  ...
  uint *input_tmp1 = const_cast<uint*>(input1);
  constexpr int vec_size1 =  sizeof(uint) / sizeof(uint);
...
void invoke_fused_elementwise_20(void* output0, const void* input0,const void* input1, int64_t batch_0,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_20<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<uint*>(output0),
        reinterpret_cast<const uint*>(input0),reinterpret_cast<const uint*>(input1),
        batch_0, 
