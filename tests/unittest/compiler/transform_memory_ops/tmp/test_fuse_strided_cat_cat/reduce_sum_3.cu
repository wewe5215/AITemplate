...
__global__ void reduce_sum_3_kernel(
...
void reduce_sum_3_launcher(
    ...
    reduce_sum_3_kernel<ElemOutputType, ElemInputType, ElementCompute, VecType>
...
void reduce_sum_3(
  ...
  if (shape[rank - 1] % 8 == 0) {
    reduce_sum_3_launcher<cutlass::half_t, cutlass::half_t, float, uint4>
      (dst_ptr, src_ptr, reduction_axis, shape, rank, stream);
    return;
  }
  if (shape[rank - 1] % 4 == 0) {
    reduce_sum_3_launcher<cutlass::half_t, cutlass::half_t, float, uint2>
      (dst_ptr, src_ptr, reduction_axis, shape, rank, stream);
    return;
  }
  if (shape[rank - 1] % 2 == 0) {
    reduce_sum_3_launcher<cutlass::half_t, cutlass::half_t, float, unsigned>
      (dst_ptr, src_ptr, reduction_axis, shape, rank, stream);
    return;
  }
  if (shape[rank - 1] % 1 == 0) {
    reduce_sum_3_launcher<cutlass::half_t, cutlass::half_t, float, cutlass::half_t>
      (dst_ptr, src_ptr, reduction_axis, shape, rank, stream);
    return;
  }
