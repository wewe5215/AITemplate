...
__global__ void
fused_elementwise_23(half* output0, const half* input0,const half* input1,const half* input2, int64_t batch_0,   int64_t n_elements) {
  ...
  constexpr int vec_size0 =  sizeof(half) / sizeof(half);
  ...
  constexpr int vec_size1 =  sizeof(half) / sizeof(half);
  ...
  half *input_tmp2 = const_cast<half*>(input2);
  constexpr int vec_size2 =  sizeof(half) / sizeof(half);
  ...
  half tmp_i2[vec_size2];
  #pragma unroll
  for (int i = 0; i < vec_size2; i++) {
    tmp_i2[i] = *input_tmp2;
  }
  const half* p_tmp_i2 = reinterpret_cast<const half*>(tmp_i2);
...
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    p_tmp_o0[i] = __hadd(__hadd(p_tmp_i0[i],p_tmp_i1[i]),p_tmp_i2[i]);
...
void invoke_fused_elementwise_23(void* output0, const void* input0,const void* input1,const void* input2, int64_t batch_0,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_23<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<half*>(output0),
        reinterpret_cast<const half*>(input0),reinterpret_cast<const half*>(input1),reinterpret_cast<const half*>(input2),
        batch_0, 
