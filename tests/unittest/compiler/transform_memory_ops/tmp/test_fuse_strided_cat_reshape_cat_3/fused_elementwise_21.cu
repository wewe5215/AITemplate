...
const int N_ELEMENTS_PER_THREAD = sizeof(uint2) / sizeof(half);
const int N_ELEMENTS_PER_READ = sizeof(uint2) / sizeof(half);
const int N_OPS_PER_THREAD = sizeof(uint2) / sizeof(half2);
...
__global__ void
fused_elementwise_21(uint2* output0, const uint2* input0,const uint2* input1, int64_t batch_0,   int64_t n_elements) {
  ...
  uint2 *input_tmp1 = const_cast<uint2*>(input1);
  constexpr int vec_size1 =  sizeof(uint2) / sizeof(uint2);
...
void invoke_fused_elementwise_21(void* output0, const void* input0,const void* input1, int64_t batch_0,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_21<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<uint2*>(output0),
        reinterpret_cast<const uint2*>(input0),reinterpret_cast<const uint2*>(input1),
        batch_0, 
