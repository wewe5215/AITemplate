...
__global__ void
fused_elementwise_5(uint4* output0,uint4* output1, const uint4* input0,   int64_t n_elements) {
...
void invoke_fused_elementwise_5(void* output0,void* output1, const void* input0,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_5<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<uint4*>(output0),reinterpret_cast<uint4*>(output1),
