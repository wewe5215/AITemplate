...
__global__ void
fused_elementwise_14(uint4* output0,uint4* output1,uint4* output2, const uint4* input0,   int64_t n_elements) {
  ...
  uint4 tmp_o1;
  half2* p_tmp_o1 = reinterpret_cast<half2*>(&tmp_o1);
  
  uint4 tmp_o2;
  half2* p_tmp_o2 = reinterpret_cast<half2*>(&tmp_o2);
  
    
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    half2 tmp_0 = relu(p_tmp_i0[i]);
p_tmp_o0[i] = tmp_0;
  ...
  *output1 = tmp_o0;
  ...
  output2 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ uint4,
                                     /*is_contiguous*/ true>(
      output2, dense_idx, 0, 0, 0);
...
void invoke_fused_elementwise_14(void* output0,void* output1,void* output2, const void* input0,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_14<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<uint4*>(output0),reinterpret_cast<uint4*>(output1),reinterpret_cast<uint4*>(output2),
