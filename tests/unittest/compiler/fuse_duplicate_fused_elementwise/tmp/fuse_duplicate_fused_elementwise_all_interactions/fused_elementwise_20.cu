...
__global__ void
fused_elementwise_20(uint4* output0, const uint4* input0,const uint4* input1,const uint4* input2,const uint4* input3,   int64_t n_elements) {
  ...
  constexpr int vec_size1 =  sizeof(uint4) / sizeof(uint4);
  
  
  input_tmp1 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ uint4,
                                     /*is_contiguous*/ true>(
      input_tmp1, dense_idx, 0, 0, 0);
  
    
  uint4 tmp_i1[vec_size1];
  #pragma unroll
  ...
  uint4 *input_tmp2 = const_cast<uint4*>(input2);
  constexpr int vec_size2 =  sizeof(uint4) / sizeof(uint4);
  
  
  input_tmp2 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ uint4,
                                     /*is_contiguous*/ true>(
      input_tmp2, dense_idx, 0, 0, 0);
  
    
  uint4 tmp_i2[vec_size2];
  #pragma unroll
  for (int i = 0; i < vec_size2; i++) {
    tmp_i2[i] = *input_tmp2;
  }
  const half2* p_tmp_i2 = reinterpret_cast<const half2*>(tmp_i2);

    

  uint4 *input_tmp3 = const_cast<uint4*>(input3);
  constexpr int vec_size3 =  sizeof(uint4) / sizeof(uint4);
  
  
  input_tmp3 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ uint4,
                                     /*is_contiguous*/ true>(
      input_tmp3, dense_idx, 0, 0, 0);
  
    
  uint4 tmp_i3[vec_size3];
  #pragma unroll
  for (int i = 0; i < vec_size3; i++) {
    tmp_i3[i] = *input_tmp3;
  }
  const half2* p_tmp_i3 = reinterpret_cast<const half2*>(tmp_i3);
...
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    p_tmp_o0[i] = __hadd2(__hadd2(__hadd2(p_tmp_i0[i],p_tmp_i1[i]),p_tmp_i2[i]),p_tmp_i3[i]);
...
void invoke_fused_elementwise_20(void* output0, const void* input0,const void* input1,const void* input2,const void* input3,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_20<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<uint4*>(output0),
        reinterpret_cast<const uint4*>(input0),reinterpret_cast<const uint4*>(input1),reinterpret_cast<const uint4*>(input2),reinterpret_cast<const uint4*>(input3),
