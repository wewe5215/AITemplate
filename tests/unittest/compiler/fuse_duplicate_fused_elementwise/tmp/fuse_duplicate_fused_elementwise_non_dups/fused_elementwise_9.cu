...
const int N_OPS_PER_THREAD = sizeof(uint4) / sizeof(half);
...
__global__ void
fused_elementwise_9(uint4* output0, const uint4* input0,   int64_t n_elements) {
...
void invoke_fused_elementwise_9(void* output0, const void* input0,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_9<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
