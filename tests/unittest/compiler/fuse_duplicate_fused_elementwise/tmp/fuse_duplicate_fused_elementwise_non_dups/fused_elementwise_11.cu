...
__global__ void
fused_elementwise_11(uint4* output0, const uint4* input0,const uint4* input1,const uint4* input2,   int64_t n_elements) {
...
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    p_tmp_o0[i] = __hadd2(__hadd2(p_tmp_i0[i],p_tmp_i1[i]),p_tmp_i2[i]);
...
void invoke_fused_elementwise_11(void* output0, const void* input0,const void* input1,const void* input2,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_11<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<uint4*>(output0),
        reinterpret_cast<const uint4*>(input0),reinterpret_cast<const uint4*>(input1),reinterpret_cast<const uint4*>(input2),
