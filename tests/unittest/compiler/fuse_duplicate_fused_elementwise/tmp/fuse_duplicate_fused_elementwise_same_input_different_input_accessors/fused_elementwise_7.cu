...
__global__ void
fused_elementwise_7(uint* output0, const uint* input0,   int64_t n_elements) {
...
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    p_tmp_o0[i] = h2sigmoid_custom(p_tmp_i0[i]);
...
void invoke_fused_elementwise_7(void* output0, const void* input0,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_7<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<uint*>(output0),
        reinterpret_cast<const uint*>(input0),
