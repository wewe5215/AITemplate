...
void slice_scatter_3(
  ...
  if (rank == 2 && num_inputs == 2) {
    int64_t local_output_shape[2];

    local_output_shape[0] = *output_shape[0];

    local_output_shape[1] = *output_shape[1];

    slice_scatter_kernel_launcher<float,
                                  2/*Rank*/,
                                  2/*NumInputs*/,
                                  256/*ElemsPerThread*/,
                                  128/*ThreadsPerBlock*/>(
        static_cast<float*>(output), 0, local_output_shape,
        reinterpret_cast<const float**>(inputs), input_shapes,
