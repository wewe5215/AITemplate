               ...
               cudaStream_t stream)
{
  ...
  // For threshold K, please refer to this post: https://fb.quip.com/HCfIAbpWB0qi
    ...
    LaunchSoftmaxSmallK<half, 32, 1>(static_cast<const half*>(input), static_cast<half*>(output), M, stream);
  

  if (!success) {
    softmaxBlockNocache<half><<<M, 1024, 0, stream>>>((half*)input, (half*)output, M, 32);
  }
