               ...
               cudaStream_t stream)
{
  ...
  // For threshold K, please refer to this post: https://fb.quip.com/HCfIAbpWB0qi
    ...
    // odd K
      ...
      // 32 < K <= 1408
      LaunchSoftmaxK1Middle<float, 51>(static_cast<const float*>(input), static_cast<float*>(output), M, stream);
    
  

  if (!success) {
    softmaxBlockNocache<float><<<M, 1024, 0, stream>>>((float*)input, (float*)output, M, 51);
  }
