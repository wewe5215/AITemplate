               ...
               cudaStream_t stream)
{
  ...
  // For threshold K, please refer to this post: https://fb.quip.com/HCfIAbpWB0qi
    ...
    LaunchSoftmaxSmallK<float, 32, 1>(static_cast<const float*>(input), static_cast<float*>(output), M, stream);
  

  if (!success) {
    softmaxBlockNocache<float><<<M, 1024, 0, stream>>>((float*)input, (float*)output, M, 32);
  }
