...
void concatenate_10_constant_folding(
  ...
  for (int64_t i = 0; i < rank; i++) {
    ...
    for (int64_t j = 1; j < num_real_inputs; j++) {
      if (real_input_shapes[j][i] != dim) {
        throw std::runtime_error(
          "invalid input shape, func_name: concatenate_10_constant_folding, dim: " +
  ...
  if (!output) {
    ...
    const TensorAccessor *input_accessors[3] = {

      &input_accessor0, &input_accessor1, &input_accessor2
  ...
  invoke_concatenate_fast<half, half, 3, 1, transform_type>(
      real_input_shapes,
      inputs,
      input_accessors,
      local_output_shape,
