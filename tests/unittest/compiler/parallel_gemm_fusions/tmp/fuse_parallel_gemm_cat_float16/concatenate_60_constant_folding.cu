...
void concatenate_60_constant_folding(
    void *output,
  ...
  for (int64_t i = 0; i < rank; i++) {
    ...
    for (int64_t j = 1; j < num_real_inputs; j++) {
      if (real_input_shapes[j][i] != dim) {
        throw std::runtime_error(
          "invalid input shape, func_name: concatenate_60_constant_folding, dim: " +
  ...
  invoke_concatenate_fast<half, half, 2, 3, transform_type>(
      real_input_shapes,
      inputs,
      input_accessors,
      local_output_shape,
      ...
      output,
      concat_dim,
      "concatenate_60_constant_folding",
      stream);
  return;
