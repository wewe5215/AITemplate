...
__global__ void
fused_elementwise_39(half* output0, const half* input0, int64_t input_batch,   int64_t n_elements) {
...
void invoke_fused_elementwise_39(void* output0, const void* input0, int64_t input_batch,   int64_t n_elements, cudaStream_t stream) {
    if (n_elements == 0) {
      return;
    }
    int block_size = static_cast<int>(std::ceil(static_cast<double>(n_elements) / N_ELEMENTS_PER_THREAD / FUSED_ELE_THREAD_SIZE));
    fused_elementwise_39<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
