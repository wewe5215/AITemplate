...
__global__ void
fused_elementwise_14(uint4* output0, const uint4* input0, int64_t input_batch,   int64_t n_elements) {
...
void invoke_fused_elementwise_14(void* output0, const void* input0, int64_t input_batch,   int64_t n_elements, cudaStream_t stream) {
    if (n_elements == 0) {
      return;
    }
    int block_size = static_cast<int>(std::ceil(static_cast<double>(n_elements) / N_ELEMENTS_PER_THREAD / FUSED_ELE_THREAD_SIZE));
    fused_elementwise_14<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
