...
__global__ void
fused_elementwise_2(uint4* output0, const uint4* input0,const uint4* input1,   int64_t n_elements) {
  ...
  input_tmp1 = get_strided_address</*data_t*/ half,
                                     /*read_t*/ uint4,
                                     /*is_contiguous*/ true>(
      input_tmp1, (dense_idx * N_ELEMENTS_PER_THREAD % (1 * 28 * 28 * 256) / (28 * 28 * 256) * (1 * 28 * 256) + dense_idx * N_ELEMENTS_PER_THREAD % (28 * 256) / (1) * (1)) / (N_ELEMENTS_PER_THREAD / (sizeof(uint4) / sizeof(uint4))), 0, 0, 0);
