...
const int N_ELEMENTS_PER_THREAD = sizeof(float) / sizeof(float);
const int N_ELEMENTS_PER_READ = sizeof(float) / sizeof(float);
const int N_OPS_PER_THREAD = sizeof(float) / sizeof(float);
...
__global__ void
fused_elementwise_3(float* output0, const float* input0, int64_t input_0_dim_1,   int64_t n_elements) {
  ...
  float *input_tmp0 = const_cast<float*>(input0);
  constexpr int vec_size0 =  sizeof(float) / sizeof(float);
  
  
  input_tmp0 = get_strided_address</*data_t*/ float,
                                     /*read_t*/ float,
                                     /*is_contiguous*/ true>(
      input_tmp0, dense_idx, 0, 0, 0);
  
    
  float tmp_i0[vec_size0];
  #pragma unroll
  for (int i = 0; i < vec_size0; i++) {
    tmp_i0[i] = *input_tmp0;
  }
  const float* p_tmp_i0 = reinterpret_cast<const float*>(tmp_i0);

    
  
  
  float tmp_o0;
  float* p_tmp_o0 = reinterpret_cast<float*>(&tmp_o0);
  
    
#pragma unroll
  for (int i = 0; i < N_OPS_PER_THREAD; ++i) {
    p_tmp_o0[i] = cosf(p_tmp_i0[i]);

  }
  
  
  
  output0 = get_strided_address</*data_t*/ float,
                                     /*read_t*/ float,
                                     /*is_contiguous*/ true>(
...
void invoke_fused_elementwise_3(void* output0, const void* input0, int64_t input_0_dim_1,   int64_t n_elements, cudaStream_t stream) {
    if (n_elements == 0) {
      return;
    }
    int block_size = static_cast<int>(std::ceil(static_cast<double>(n_elements) / N_ELEMENTS_PER_THREAD / FUSED_ELE_THREAD_SIZE));
    fused_elementwise_3<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<float*>(output0),
        reinterpret_cast<const float*>(input0),
        input_0_dim_1, 
