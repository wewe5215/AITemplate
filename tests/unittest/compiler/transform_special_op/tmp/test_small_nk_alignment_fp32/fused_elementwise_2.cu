...
const int N_ELEMENTS_PER_THREAD = sizeof(uint2) / sizeof(float);
const int N_ELEMENTS_PER_READ = sizeof(uint2) / sizeof(float);
const int N_OPS_PER_THREAD = sizeof(uint2) / sizeof(float);
...
__global__ void
fused_elementwise_2(uint2* output0, const uint2* input0, int64_t input_0_dim_0,   int64_t n_elements) {
  ...
  uint2 *input_tmp0 = const_cast<uint2*>(input0);
  constexpr int vec_size0 =  sizeof(uint2) / sizeof(uint2);
...
void invoke_fused_elementwise_2(void* output0, const void* input0, int64_t input_0_dim_0,   int64_t n_elements, cudaStream_t stream) {
    ...
    fused_elementwise_2<<<block_size, FUSED_ELE_THREAD_SIZE, 0, stream>>>(
        reinterpret_cast<uint2*>(output0),
        reinterpret_cast<const uint2*>(input0),
        input_0_dim_0, 
